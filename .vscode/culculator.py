...
BOT_NAME = 'calc_bot'
bot = telebot.TeleBot('1495786616:AAHsIzgD05wrs_A7lBiam0IUpLviI9ivmJ0')

TIMEOUT_CONNECTION = 5  # Таймаут переподключения

# Ссылка на иконку "c выражением"
WITHOUT_ICON = 'https://raw.githubusercontent.com/6eremotuk01/Calculator-bot/master/img/without.jpg'
# Ссылка на иконку "без выражения"
WITH_ICON = 'https://raw.githubusercontent.com/6eremotuk01/Calculator-bot/master/img/with.jpg'
...
def send_start(message):
    ...
    if message.text.lower() == '/start':
        msg = bot.send_message(
            message.chat.id, START_MESSAGE, parse_mode='markdown')

    elif message.text.lower() == '/help':
        msg = bot.send_message(
            message.chat.id, HELP_MESSAGE, parse_mode='markdown')
...
def answer_to_user(message):
    ...
    if user_message == 'привет':
        msg = bot.send_message(message.chat.id, '*Привет, %s*' %
                               (message.chat.first_name), parse_mode='markdown')
        ...
        try:
            ...
            msg = bot.send_message(
                message.chat.id, user_message.replace(' ', '') + ' = ' + answer)

        except SyntaxError:
            msg = bot.send_message(
                message.chat.id, 'Похоже, что вы написали что-то не так. \nИсравьте ошибку и повторите снова')
        except NameError:
            msg = bot.send_message(
                message.chat.id, 'Переменную которую вы спрашиваете я не знаю. \nИсравьте ошибку и повторите снова')
        except TypeError:
            msg = bot.send_message(
                message.chat.id, 'Мне кажется, что в выражении присутствует ошибка типов. \nИсравьте ошибку и повторите снова')
...
if (__name__ == '__main__'):
    while True:
        try:
            ...
            print('Ошибка подключения. Попытка подключения через %s сек.' %
                  TIMEOUT_CONNECTION)
